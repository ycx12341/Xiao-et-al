...
# used in the ABC-BCD scheme which applies to the main reference
# dataset in the second run.
...
set.seed(123) # A different set.seed is used to avoid obtaining the same initial parameters.
dn <- runif(10000,0.000069,0.02)
gamma <- runif(10000,0.005,0.26)
eta <- paras_ecm_r4[,3]
dm <- runif(10000,0.0001,0.033)
...
set.seed(121) # Another set.seed to avoid obtaining the same initial parameters. 
dn <- runif(10000,0.000069,0.02)
gamma <- runif(10000,0.005,0.26)
